...
describe("Migration 002 (workspaces)", () => {
  it("applies after 001 and creates the workspaces table + repo column", () => {
    const db = newDb();
    ...
    const reposCols = db.prepare(`PRAGMA table_info(${REPOS_TABLE})`).all() as Array<{ name: string }>;
    expect(reposCols.map((c) => c.name)).toContain("workspace_id");
    db.close();
  });

  it("seeds a default workspace for the default tenant", () => {
    const db = newDb();
    new MigrationRunner(db, "sqlite").migrate();
    const rows = db
  ...
  it("backfills pre-existing repos to the tenant's default workspace", () => {
    ...
    migration002.up({ db, dialect: "sqlite" });
  ...
  it("re-running the migration is idempotent (no duplicate workspaces, no column errors)", () => {
    const db = newDb();
