...
export class KnowledgeStore {
  ...
  removeNode(id: string): void {
    this.db.prepare("DELETE FROM knowledge_edges WHERE (source_id = ? OR target_id = ?) AND tenant_id = ?").run(id, id, this.tenantId);
  ...
  // --- Edge CRUD ---
  addEdge(sourceId: string, targetId: string, relation: EdgeRelation, weight: number = 1.0, metadata?: Record<string, unknown>): void {
    this.db.prepare(
      "INSERT OR REPLACE INTO knowledge_edges (source_id, target_id, relation, weight, metadata, tenant_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)"
    ).run(sourceId, targetId, relation, weight, JSON.stringify(metadata ?? {}), this.tenantId, new Date().toISOString());
  }

  removeEdge(sourceId: string, targetId: string, relation?: EdgeRelation): void {
    if (relation) {
      this.db.prepare("DELETE FROM knowledge_edges WHERE source_id = ? AND target_id = ? AND relation = ? AND tenant_id = ?").run(sourceId, targetId, relation, this.tenantId);
    } else {
      this.db.prepare("DELETE FROM knowledge_edges WHERE source_id = ? AND target_id = ? AND tenant_id = ?").run(sourceId, targetId, this.tenantId);
    }
  }
  ...
  // --- Traversal ---
  neighbors(id: string, opts?: { relation?: EdgeRelation; direction?: "out" | "in" | "both"; maxDepth?: number; types?: NodeType[] }): KnowledgeNode[] {
