 ...
 * an EC2 row (compute_kind='ec2') and asserting that a `StopFailure` hook
...
describe("hook flow retry on EC2 (smoke)", () => {
  ...
  it("hook StopFailure on EC2 session triggers on_failure retry", async () => {
    ...
    // Seed an EC2 compute row. `provider: "ec2"` resolves to
    // compute_kind='ec2' via providerToPair, which is all the retry path
    // reads.
