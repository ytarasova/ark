...
function writeTranscript(name: string, cwd: string, lines: object[]): string {
  ...
  const content = [{ type: "session_meta", payload: { cwd } }, ...lines].map((l) => JSON.stringify(l)).join("\n");
  ...
  it("extracts the LAST cumulative token_count event (Codex emits incremental totals)", () => {
    const path = writeTranscript("rollout-cumulative.jsonl", "/tmp/wd1", [
      { type: "turn_context", payload: { model: "gpt-5-codex" } },
      {
        ...
        payload: {
          type: "token_count",
          info: { total_token_usage: { input_tokens: 100, output_tokens: 50, cached_input_tokens: 10 } },
        },
      },
      {
        type: "event_msg",
        payload: {
          type: "token_count",
          info: { total_token_usage: { input_tokens: 500, output_tokens: 300, cached_input_tokens: 80 } },
        },
      },
    ]);
    ...
    const path = writeTranscript("rollout-reasoning.jsonl", "/tmp/wd2", [
      {
        ...
        payload: {
          type: "token_count",
          info: { total_token_usage: { input_tokens: 10, output_tokens: 20, reasoning_output_tokens: 5 } },
        },
      },
    ]);
    const result = parser.parse(path);
