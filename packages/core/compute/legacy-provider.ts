/**
 * Legacy `ComputeProvider` interface (single-axis dispatch model).
 *
 * Survives until Task 5 of the compute cleanup, which sweeps the remaining
 * `app.getProvider()` callers (CLI, `cleanupSession`, `buildPlacementCtx`)
 * and deletes both the registry methods on AppContext and this file.
 *
 * New code should target `Compute` + `Isolation` from `./types.js` instead.
 */
