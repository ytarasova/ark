...
export class K8sProvider implements ComputeProvider {
  ...
  private async buildKubeConfigFromCluster(
    ...
    const { resolveEffectiveClusters } = await import("../../config/clusters.js");
  ...
  async launch(compute: Compute, session: Session, opts: LaunchOpts): Promise<string> {
    ...
    if (cfg.credsSecretName) {
      try {
        const { setSecretOwnerToPod } = await import("../../services/dispatch-claude-auth.js");
        await setSecretOwnerToPod(this.app, {
          ...
          api: api as unknown as import("../../services/dispatch-claude-auth.js").K8sSecretsApi,
        });
      } catch (e: any) {
