/**
 * Lazily build a DrizzleClient from an IDatabase handle.
 *
 * Used by repositories during the drizzle cutover (Phase B): every repo
 * keeps its existing `constructor(db: IDatabase)` signature so no caller
 ...
 * IDatabase already owns, so there is no second pool or second SQLite
 * handle. When the IDatabase is closed, the drizzle handle dies with it.
 *
...
export function drizzleFromIDatabase(db: IDatabase): DrizzleClient {
  ...
  if (db instanceof BunSqliteAdapter) {
    ...
    throw new Error(
      "drizzleFromIDatabase: unsupported IDatabase implementation " +
