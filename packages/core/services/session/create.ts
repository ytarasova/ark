...
export class SessionCreator {
  ...
  async start(opts: StartSessionOpts, hooks?: LifecycleHooks): Promise<Session> {
    ...
    if (mergedOpts.workspace_id) {
      ...
      const ws = await d.codeIntel.getWorkspace(wsId);
      if (!ws) {
        throw new Error(`workspace ${wsId} not found; cannot dispatch session ${session.id}`);
      }
      const reposInWs = opts.repo ? await d.codeIntel.listReposInWorkspace(ws.tenant_id, ws.id) : [];
      const primaryRepoId =
        opts.repo && ws
          ? (reposInWs.find(
              (r) => r.name === opts.repo || r.local_path === opts.repo || r.repo_url === opts.repo,
            )?.id ?? null)
          : null;
