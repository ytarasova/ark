/**
 * /agent/intervention + /agent/interventions/stream tests. Drives a live
...
import { _resetForTests } from "../routes/interventions.js";
...
describe("/agent/intervention", () => {
  test("buffers messages until a stream subscribes, then drains FIFO", async () => {
    ...
    const r1 = await fetch(`${baseUrl}/agent/intervention?session=ark-s-1`, {
      method: "POST",
    ...
    const r2 = await fetch(`${baseUrl}/agent/intervention?session=ark-s-1`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const resp = await fetch(`${baseUrl}/agent/interventions/stream?session=ark-s-1`, { signal: ac.signal });
  ...
  test("hands directly to a parked consumer (delivered=true)", async () => {
    const ac = new AbortController();
    const respPromise = fetch(`${baseUrl}/agent/interventions/stream?session=ark-s-2`, { signal: ac.signal });
    // Give arkd a moment to register the parked waiter before the publish.
    await new Promise((r) => setTimeout(r, 25));

    const r = await fetch(`${baseUrl}/agent/intervention?session=ark-s-2`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  test("multi-session isolation: A's queue is untouched while B drains", async () => {
    await fetch(`${baseUrl}/agent/intervention?session=ark-s-A`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ content: "for A" }),
    });
    await fetch(`${baseUrl}/agent/intervention?session=ark-s-B`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const ac = new AbortController();
    const resp = await fetch(`${baseUrl}/agent/interventions/stream?session=ark-s-B`, { signal: ac.signal });
    const lines = await readNdjsonLines(resp, 1, ac);
    expect(lines).toEqual([{ content: "for B" }]);

    // A's queue should still hold its message, retrievable on its own subscription.
    const ac2 = new AbortController();
    const resp2 = await fetch(`${baseUrl}/agent/interventions/stream?session=ark-s-A`, { signal: ac2.signal });
  ...
  test("rejects missing/invalid session param", async () => {
    const r1 = await fetch(`${baseUrl}/agent/intervention`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const r2 = await fetch(`${baseUrl}/agent/intervention?session=has spaces`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ content: "x" }),
    });
  ...
  test("rejects empty content", async () => {
    const r = await fetch(`${baseUrl}/agent/intervention?session=ark-s-3`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  test("rejects unknown control values", async () => {
    const r = await fetch(`${baseUrl}/agent/intervention?session=ark-s-4`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
