...
export function registerRuntimeCommands(program: Command) {
  ...
  runtime.command("list").description("List available runtimes").action(async () => {
    ...
    for (const r of runtimes) {
      ...
      console.log(`  ${src} ${(r.name ?? "").padEnd(12)} ${(r.type ?? "").padEnd(12)} models: ${models}  ${r.description ?? ""}`);
    }
  });

  runtime.command("show").description("Show runtime details").argument("<name>").action(async (name) => {
    const r = getApp().runtimes.get(name);
    if (!r) { console.log(chalk.red(`Runtime '${name}' not found`)); return; }
