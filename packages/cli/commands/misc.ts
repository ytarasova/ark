...
export function registerMiscCommands(program: Command) {
  ...
  // ── Agent SDK launcher (invoked as a child process by the agent-sdk executor) ─
  // This subcommand is used in compiled-binary mode. Dev mode spawns launch.ts
  // directly. Both paths read all context from ARK_* env vars.
