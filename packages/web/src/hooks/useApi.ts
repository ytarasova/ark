...
/**
 * Typed client surface over a `WebTransport`. All JSON-RPC methods flow through
 * `transport.rpc()`; there is no module-level singleton. Get an instance from
 * the `useApi()` hook (recommended for React call sites) or call `makeApi()`
 * directly if you have a transport in hand.
 */
export type ApiClient = ReturnType<typeof makeApi>;

/**
 * Factory that binds the full typed API surface to a given transport.
 * Every method delegates to `transport.rpc()` -- so swapping transports (e.g.
...
export function makeApi(transport: WebTransport) {
  ...
  return {
    ...
    gateApprove: (id: string) =>
      rpc<GateApproveResponse>("gate/approve", { sessionId: id } satisfies GateApproveRequest),
    ...
    getTriggers: (tenant?: string) =>
      rpc<{ triggers: any[] }>("trigger/list", { tenant }).then((r) => r.triggers ?? []),
    getTriggerSources: () =>
...
/**
 * React hook returning a typed API client bound to the transport provided by
 * `TransportProvider`. Memoised on `transport` identity so swapping transports
 * (tests + admin scopes) produces a fresh client; re-renders with the same
 * transport return the same instance (stable query keys, stable callbacks).
 *
 * Prefer this over reaching for a module singleton -- each render scope gets
 * its own transport-bound client, which makes side-by-side `<TransportProvider>`
 * trees fully isolated (tests rely on this).
 */
export function useApi(): ApiClient {
  const transport = useTransport();
  return useMemo(() => makeApi(transport), [transport]);
}
