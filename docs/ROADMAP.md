...
> - **Tauri v2 desktop (v0.17.0)** -- Electron replaced by Tauri v2 under `packages/desktop/` (Rust + tauri v2 + plugin-shell + plugin-opener + single-instance). 29x smaller installer (3.2 MB vs 94.3 MB). Makefile targets `desktop` / `build-desktop` / `package-desktop`. CI job `tauri-build` across macOS arm64, Linux x64, Windows x64. Release pipeline builds Tauri bundles.
...
| ~~Tauri desktop scaffold~~ | **DONE (v0.17.0, 2026-04-15)** -- Electron replaced by Tauri v2 under `packages/desktop/`. 29x smaller installer (3.2 MB DMG). Release pipeline builds Tauri bundles (DMG, deb, AppImage, exe). `make desktop` / `make build-desktop` / `make package-desktop`. | 0.5 day | done |
| ~~Desktop app distribution -- make Tauri the default~~ | **DONE (v0.17.0, 2026-04-15)** -- `release.yml` swapped to Tauri, Electron deleted, `packages/desktop-tauri/` moved to `packages/desktop/`. Desktop E2E tests need porting (Electron Playwright -> Tauri WebDriver). | 1 day | done |
