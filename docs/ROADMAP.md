...
| **Unified Claude settings bundle** | `writeSettings` (formerly `writeHooksConfig`) writes hooks, `permissions.allow` from `agent.tools`, auto-expanding `mcp_servers` to wildcards, rejecting undeclared MCP refs. Tool-hint injection appends an "Available tools" block to the system prompt so agents don't probe. `--dangerously-skip-permissions` stays the explicit override. Typed via `ClaudeSettingsOpts`. | 18 tests |
