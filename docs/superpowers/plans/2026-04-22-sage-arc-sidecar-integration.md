...
| Mid-turn correction (`session/interrupt`) | "Stop, do X instead" while agent is mid-generation | **Shipped (D3):** writes `{"role":"user","content":"...","control":"interrupt"}` to `interventions.jsonl`; tail calls `onInterrupt()` -> `abortHolder.ref.abort()`; `drainStream` detects `interruptFlag.fired` -> returns `outcome: "interrupted"`; outer loop resumes with `options.resume = <sdkSessionId>` so SDK restores conversation history and sees the correction as the next user message. | Phase 1 -- DONE |
...
| Hard kill (`session/kill`) | SIGTERM ignored | **Shipped (D3):** SIGKILL via `executor.terminate()` (no grace), marks session `failed` with reason `killed`, runs D2 cleanup synchronously. Returns `{ ok: true, terminated_at, cleaned_up: true }`. | Phase 1 -- DONE |
...
**Mid-turn interrupt + resume is shipped (D3, 2026-04-22).** `session/interrupt { sessionId, content }` appends `{"role":"user","content":"...","control":"interrupt","ts":<ms>}\n` to the same file. The tail detects `control === "interrupt"`, calls `onMessage(content)` (pushes correction to queue) then `onInterrupt()` (sets `interruptFlag.fired = true` and calls `abortHolder.ref.abort()`). The outer loop in `launch.ts` catches the abort, sees `interruptFlag.fired`, returns `outcome: "interrupted"`, and re-enters the while loop with `options.resume = sdkSessionId`. SDK option `resume` restores conversation history so the agent sees the correction as the next user message. Up to 20 interrupt-resume cycles per session lifetime (`MAX_INTERRUPTS`). SDK session ID is captured from the first `system/init` message and held across iterations.

**Hard kill is shipped (D3, 2026-04-22).** `session/kill { sessionId }` calls `executor.terminate(handle)` (new method on `agentSdkExecutor`, SIGKILL-first with no SIGTERM grace, awaits process exit for clean post-conditions), marks session `failed` with `error: "killed"`, then runs `cleanupSession()` synchronously. Returns `{ ok: true, terminated_at: <ms>, cleaned_up: true }`. Terminal sessions return `{ ok: false }`. Unknown sessions throw `SESSION_NOT_FOUND`. Non-agent-sdk executors that don't implement `terminate` fall back to `kill(handle)` (defined as optional on the `Executor` interface).
