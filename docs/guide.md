...
| `claude-agent` | Anthropic Agent SDK (in-process, direct or gateway-routed) | api | agent-sdk |
...
# Use the in-process Agent SDK (no CLI, runs in the conductor process)
ark session start --repo . --summary "Bulk rename" \
...
- `claude-agent` -- runs the Anthropic Agent SDK in-process. Hooks stream via arkd `/hooks/forward` to the conductor. Supports gateway routing (e.g. Bedrock).
