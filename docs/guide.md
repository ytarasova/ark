...
| -------------- | ----------------------------- | --------------------------- | ---------------------------------- |
| `claude-agent` | Anthropic Agent SDK (in-proc) | api (per token)             | claude                             |
...
- `claude-agent` -- runs the Anthropic Agent SDK in-process (no external CLI); hooks stream via arkd for remote compute.
